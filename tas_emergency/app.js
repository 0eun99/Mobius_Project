...
const button = new Gpio(20, 'in', 'falling', {debounceTimeout: 30});   // GPIO16 (pin): button input
const LED = 21;
const SPEAKER = 4;
...
button.watch((err, value) => {
    ...
    const led = new Gpio(LED, 'out');
    const speaker = new Gpio(SPEAKER, 'out');

    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].ctname == 'emergency') {
    ...
    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].ctname == 'emergency') {
                var cin = { ctname: upload_arr[i].ctname, con: 0 };
                console.log('SEND : ' + JSON.stringify(cin) + ' ---->');
                upload_client.write(JSON.stringify(cin) + '<EOF>');
                break;
            }
